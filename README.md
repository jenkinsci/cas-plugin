...
When a CAS server returns multiple roles as one delimited attribute value, configure **Roles Attribute Value Separator** for the selected protocol. The separator is disabled by default, so values containing commas (such as LDAP distinguished names) remain unchanged.
